...
class RationalNeuralFingerprintOutput(tf.keras.layers.Layer):
    ...
    def call(self, inputs, mask = None):
        ...
        if self.use_rational_hidden_activation == True:
            ...
            neural_fp_atomwise = self.D_1(summed_atom_bond_features)
...
class DeepRationalNeuralFingerprintHidden(tf.keras.layers.Layer):
    ...
    def build(self, inputs_shape):
        ...
        if self.use_rational_activation == True:
            ...
            self.D_5 = Dense(units = self.conv_width, use_bias = self.use_bias, activation = None, kernel_initializer = self.kernel_initializer, bias_initializer = self.bias_initializer)
            ...
            self.R_5 = RationalLayer(shared_axes = self.rational_shared_axes, alpha_initializer = self.rational_alpha_initializer, beta_initializer = self.rational_beta_initializer)
            ...
            self.D_5 = Dense(units = self.conv_width, use_bias = self.use_bias, activation = self.activation, kernel_initializer = self.kernel_initializer, bias_initializer = self.bias_initializer)
    ...
    def call(self, inputs, mask=None):
        ...
        if self.use_rational_activation == True:
            ...
            new_features = self.D_1(summed_atom_bond_features)
            ...
            new_features = self.D_5(new_features)
            new_features = self.R_5(new_features)
